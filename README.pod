...
(supported are cvs, svn, git, hg, bzr and darcs development packages, as well
as nightly build packages).
...
development packages (supported are svn, git, hg and bzr development packages)
that are found up-to-date after the source checkout will be skipped.
